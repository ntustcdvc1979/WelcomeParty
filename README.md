# 相見歡

台科大崇德志工社相見歡的現場互動簡報。投影幕跑流程、主持人用手機當後臺遙控，
三隻猴子的題目只出現在比劃猴的手機上。純靜態網頁，由 GitHub Actions 部署到 GitHub Pages。

**線上網址** <https://ntustcdvc1979.github.io/WelcomeParty/>

| 頁面 | 給誰 |
|---|---|
| [`stage.html`](stage.html) | 投影幕。接投影機的那台，主持人用鍵盤操作。 |
| [`admin.html`](admin.html) | 後臺。遙控翻頁、管三隻猴子的題目、填抽獎名單與獎項。要用核可過的 Google 帳號登入，**不要投出去**。 |
| [`phone.html`](phone.html) | 比劃猴的手機。只顯示目前的題目。 |
| [`index.html`](index.html) | 入口。三個頁面的連結、QR code、活動前的檢查清單。 |

## 流程

| | 段落 | 現場在做什麼 |
|---|---|---|
| | 首頁 | 社徽、社名、相見歡 |
| 📖 | **活動介紹** | 開場大標，各種活動的照片散在四周 |
| 🙈 | **三隻猴子** | 三人一組：比劃猴看手機比手畫腳、傳話猴用嘴巴描述、猜題猴閉眼把答案畫出來。題目由後臺換，投影幕只顯示第幾題與計時 |
| 🤝 | **社團介紹** | 環保、生命教育、STEAM 科學營、創藝營隊、紓壓手做、甜蜜食光，六頁照片拼貼。版面照 0915 茶會那份 pptx 排的，另外配了手繪裝飾 |
| 🚑 | **賑災** | 醫療關懷隊在災區（沿用迎新第 48 頁） |
| ✈️ | **海外交流** | 海外義診與越南惠安，配上飛機與椰子樹裝飾 |
| 🧳 | **社遊** | 平溪天燈、擎天崗、野餐、蔬適 BBQ、生菜捲。拼貼排版照茶會簡報初稿 |
| 🍲 | **蔬食料理社聚** | 伙食團平常的樣子（沿用迎新第 42 頁） |
| 💳 | **社員權益** | 社費與三項社員福利 |
| 🔗 | **這學期的活動** | 八張宣傳圖（照活動日期排）、報名傳送門 QR、集章說明。點宣傳圖可以放大 |
| 📝 | **心得回饋** | 只有標題和回饋表單 QR（`assets/feedback-qr.webp`） |
| 📸 | **大合照** | 按 <kbd>T</kbd> 跑 3・2・1・📸 |
| 🎁 | **抽獎** | 後臺貼名單與獎項，投影幕跑馬燈隨機抽，中獎者自動從名單移除。同一頁附問卷 QR |

三隻猴子的題目**不會出現在投影幕上**。全場都不知道答案，猜題猴畫完才公布，
笑點才夠大——這也是為什麼比劃猴要用手機看題目。

投影幕上每一段文字都是中英雙語：中文一行，英文小一號排在下面。
中文改了記得把對應的 `en` 欄位一起改，不然英文會留在舊的說法。

## 開始之前

**必看** [`docs/SETUP.md`](docs/SETUP.md)：Firebase 設定、部署、現場操作、
以及網路出事時的備援作法。

最短路徑：

1. 建 Firebase 專案，開 Realtime Database。
2. 把設定存成 repo secret **`FIREBASE_CONFIG`**（JSON 格式），
   Settings → Pages → Source 選 **GitHub Actions**。push 之後會自動部署。
3. 檢查 [`assets/photos-data.js`](assets/photos-data.js) 的照片與文案。

沒設定 Firebase 也不會壞：投影幕會自動退到本機模式，鍵盤照樣跑完全部畫面，
只是後臺和手機不會同步。

## 鍵盤（投影幕）

<kbd>→</kbd> 下一頁　<kbd>←</kbd> 上一頁　<kbd>Esc</kbd> 跳頁選單　<kbd>F</kbd> 全螢幕

<kbd>T</kbd> 三隻猴子計時 90 秒／抽一位／大合照倒數　<kbd>R</kbd> 重設本頁　<kbd>M</kbd> 配樂靜音

## 檔案

```
stage.html                     投影幕，流程的唯一真相來源
admin.html                     後臺
phone.html                     比劃猴的手機
index.html                     入口與檢查清單
assets/
  settings.js                  房號、社群連結等不敏感設定（進 git）
  firebase-config.sample.js    Firebase 設定範本
  firebase-config.js           實際設定，.gitignore 掉，由 CI 從 secret 產生
  net.js                       連線層：firebase → 本機模式自動降級
  theme.css                    色票與共用元件
  monkeys-data.js              三隻猴子的三個角色與預設題庫
  photos-data.js               社團介紹的拼貼座標，與三面照片牆的清單
  qrcode.js                    自製 QR 產生器，不依賴任何外部服務
  logo1.webp logo2.webp        兩顆社徽
  feedback-qr.webp             心得回饋表單的 QR
  volunteer.mp3                全場配樂（<kbd>M</kbd> 靜音）
  midmoon.webp 等八張           這學期的活動宣傳圖（檔名對應活動，順序在 stage.html 的 PROMOS）
  calendar.webp                這學期的行事曆
media/                         老師照片放這裡
media/photos/                  社團介紹與照片牆的圖檔，一律 webp
docs/SETUP.md                  設定與現場操作手冊
docs/database.rules.json       Firebase 資料庫規則，直接貼進主控台
.github/workflows/deploy.yml   部署，順便把 secret 寫成 firebase-config.js
```

要換題目、換老師、換照片，動 `assets/*-data.js` 就好。
題庫還可以直接在後臺改，改完存在資料庫裡，不用重新部署。

## 社團介紹那幾頁是怎麼排的

照片的位置直接抄 0915 茶會那份 `封面.pptx`：每一張的 `l/t/w/h` 是佔投影片寬高的
百分比（原稿 20 x 11.25 吋，剛好 16:9），所以在任何解析度下都跟原本的簡報長得一樣。
有幾張本來就是刻意滿出版面的，那不是算錯。

裝飾（太陽、海浪、狗掌、燒杯、水火箭、投石機、花、毛球、甜甜圈、提拉米蘇、
草叢、楊枝甘露、飛機、椰子樹）是畫的 SVG，放在 `stage.html` 的 `DECO` 裡，
位置一樣用百分比寫在 `photos-data.js`。不用 emoji 是因為它在不同系統長得不一樣，
顏色也跟社團色票對不起來。

照片牆那幾頁的 `deco` 座標是**整個畫面**的百分比，不是拼貼容器的——
那幾頁沒有拼貼容器，裝飾直接貼在畫面上。

指導老師那一段已經從流程拿掉（社團有自己的介紹 PPT）。
`VIEWS.teacher` 和 `teachers-data.js` 還留著，要加回去只是在流程裡補兩行。

pptx 裡的「花蓮堰塞賑災」「越南」「社遊」三頁只有標題沒有照片，
所以賑災與國外交流改接迎新那兩面照片牆，社遊先沒做。

賑災原本是一張九宮格拼貼，每一格都小到看不清楚，所以沿著白底切成八張分開放，
等高排版就會把每一張撐大。其中大合照、量血壓、五人合影三張已經換成原始檔，
剩下五張還是從那張拼貼圖切出來的（約 220×165），找得到原檔的話換掉會更清楚。

賑災、海外交流、社遊這幾頁只留標題，把版面整個讓給照片。

三隻猴子規則那一頁字最多，中英兩份一起排就得縮到後排看不清楚，
所以改成一次只顯示一種語言，用畫面上的按鈕或 <kbd>L</kbd> 鍵切換。

照片牆預設是貪心排法——能塞幾張就塞幾張，所以六張會排成 4+2、五張會擠成一列。
在 `photos-data.js` 給那一組加 `perRow`，就會照指定的張數分列（海外、社遊、開伙都是 3）。

照片牆的標題與說明排成同一列，不上下疊。英文小標則排在中文下面。
排完之後容器會縮成內容的實際高度，標題和照片才會一起置中，中間不留空白。

蔬食料理社聚那一組設了 `fill`，改用等分格線加 `object-fit:cover` 填滿整塊。
等高排版填不滿寬度——照片比例固定，排滿高度就一定留白邊——代價是左右會裁掉一點。

首頁在大合照後和抽獎後各重播一次，中間空檔投著就好。

社團介紹那幾頁的標題字級綁的是容器**寬度**（`5cqw`，剛好是 pptx 的 72pt ÷ 20 吋），
不是高度。綁高度的話接到 4:3 投影機時標題會相對變寬，壓到旁邊的照片。
標題也設了 `white-space:nowrap`——折行會往下掉到照片上，寧可讓它往右邊的留白超出去。

## 資料怎麼流的

房間底下有五個節點，誰寫誰讀分得很清楚：

| 節點 | 誰寫 | 誰讀 |
|---|---|---|
| `outline` | 投影幕 | 後臺（跳頁下拉） |
| `state` | 投影幕 | 後臺、手機 |
| `control` | 後臺 | 投影幕 |
| `monkey` | 後臺 | 投影幕、手機 |
| `lottery/roster` `prizes` `prizeAt` `draw` | 後臺 | 投影幕 |
| `lottery/history` | 投影幕 | 兩邊 |

抽獎的亂數故意放在投影幕：跑馬燈停在誰身上，跟寫進資料庫的中獎者必須是同一個人。
讓後臺先抽好再叫投影幕演，中間斷線就會對不起來。

「還沒中獎的人」是 `roster` 減掉 `history` 算出來的，不另外存一份。
這樣投影幕只需要寫 `history`，名單本身就能鎖起來只讓登入過的管理員改
——不然投影幕沒登入，名單那個節點就得對所有人開放寫入。

## 後臺登入

後臺有題庫和抽獎名單，所以要 Google 登入，而且要社團核可過的帳號。
名單存在資料庫的 `admins/<uid>`，不進版本庫，所以幹部的 email 不會公開在 repo 上。

第一個管理員要在 Firebase 主控台手動加一筆（步驟在 SETUP 第 1.5 節），
之後新人自己登入一次，現任管理員在後臺按一下核可就好。

投影幕和比劃猴的手機**不用登入**——現場多一道登入就多一個會卡住的地方。
本機模式（沒設定 Firebase）整套登入都不會啟動。

## 關於那把 Firebase API key

Firebase 的 web API key 本來就不是密碼——它只識別專案，不授權存取，部署後在瀏覽器裡
本來就看得到。真正的防線是資料庫規則和活動後刪資料。這個 repo 仍然不把它寫進版本庫，
是為了不讓爬蟲從公開原始碼掃走亂打流量。詳見 SETUP.md 第二節。

## 資料與隱私

抽獎名單是主持人自己貼上去的，只有名字。不收系級、電話或任何聯絡方式。
資料存在社團自己的 Firebase 專案，活動結束後請照 SETUP.md 最後一節把它刪掉。
